...
import { deployToDevnet } from "@repo/web3/src/cli";
...
// 			alphAmount: 10n ** 18n,
// 			tokens: [{ id: testTokenId, amount: 9n }],
// 		});
...
// 			nodeProvider: new NodeProvider("http://127.0.0.1:22973"),
// 		});

// 		const collection = deployments.getInstance(NFTCollection, 0);
// 		if (collection === undefined) {
// 			console.log(`The contract is not deployed on group ${signer.group}`);
// 			throw new Error("The contract is not deployed on group");
// 		}
...
// 			);
// 		const [event] = events;
// 		const [, , nftId] = event.fields;

// 		const nftInstance = NFT.at(addressFromContractId(nftId.value as string));
// 		console.log({ nftInstance });

// 		const tokenUri = await nftInstance.methods.getTokenUri();
